...
public class OtelApiStack : Stack
{
    ...
    private Instance_ CreateEc2Instance(IVpc vpc, SecurityGroup sg, Role role, Amazon.CDK.AWS.APS.CfnWorkspace ampWorkspace, CfnKeyPair keyPair)
    {
        ...
        userData.AddCommands(
            ...
            "Write-Host '",
            "[2/6] Installing .NET 10 Hosting Bundle...' -ForegroundColor Yellow",
            "",
            "# Correct URL from Microsoft builds",
            "$dotnetUrl = 'https://download.visualstudio.microsoft.com/download/pr/df7b90e1-2b1e-4ec5-a2b3-7b3e5e5e5e5e/5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e/dotnet-hosting-10.0.1-win.exe'",
            ...
            "        Invoke-WebRequest -Uri $dotnetUrl -OutFile $dotnetInstaller -UseBasicParsing -TimeoutSec 600",
            "        $downloadSuccess = $true",
            "        Write-Host '.NET 10 Hosting Bundle downloaded successfully' -ForegroundColor Green",
            "    } catch {",
            ...
            "if ($downloadSuccess) {",
            "    try {",
            "        Write-Host 'Installing .NET 10 Hosting Bundle...' -ForegroundColor Cyan",
            "        $installProcess = Start-Process -FilePath $dotnetInstaller -Args '/install /quiet /norestart' -Wait -NoNewWindow -PassThru",
            "        ",
            ...
            "    }",
            "} else {",
            "    Write-Host 'Skipping installation due to download failure' -ForegroundColor Red",
            "}",
            "",
            ...
            "# Verify installation",
            "Write-Host '",
            "Verifying .NET installation:' -ForegroundColor Cyan",
            "if (Get-Command dotnet -ErrorAction SilentlyContinue) {",
            "    dotnet --version",
            "    dotnet --list-runtimes",
            "    Write-Host '.NET verification successful!' -ForegroundColor Green",
            "} else {",
            "    Write-Host '.NET installation verification FAILED - dotnet command not found!' -ForegroundColor Red",
            "}",
            "",
            ...
            "<html>",
            "<head>",
            "    <title>Observability API</title>",
            "    <meta property=\"og:title\" content=\"MDaudit Enterprise Solutions\">",
            "    <meta property=\"og:type\" content=\"website\">",
            "    <meta property=\"og:description\" content=\"Healthcare auditing enterprise platform\">",
            "</head>",
    ...
    private Amazon.CDK.AWS.Grafana.CfnWorkspace CreateAmgWorkspace()
    {
        ...
        var amgWorkspace = new Amazon.CDK.AWS.Grafana.CfnWorkspace(this, "ObservabilityGrafanaWorkspace", new Amazon.CDK.AWS.Grafana.CfnWorkspaceProps
        {
            ...
            AuthenticationProviders = ["AWS_SSO"],  // Keep SSO for now
